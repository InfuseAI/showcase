...
class PycaretClassificationTraining:
    ...
    def automl_training(self, target_column, mlflow_experiment_name):
        # Initializing setup
        clf1 = setup(self.df, target = target_column, log_experiment = True, experiment_name = mlflow_experiment_name, silent = True)
    ...
    def test_model(self, model_path):
        ...
        data_unseen = pd.DataFrame(test_data, columns=['age', 'job', 'marital', 'education', 'default', 'balance', 'housing', 'loan', 'contact', 'day', 'month', 'duration', 'campaign', 'pdays', 'previous', 'poutcome', 'deposit'])
